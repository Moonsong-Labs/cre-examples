...
import { packVols, packCorrs } from "./lib/pack";
import { getBootstrapReturns } from "./lib/bootstrap";
import { ReportDataABI } from "./abi/RiskMetricsOracle";
...
const onCronTrigger = (runtime: Runtime<Config>): string => {
  ...
  for (let i = 0; i < priceData.prices.length; i++) {
    runtime.log(`  ${AssetNames[i]}: ${priceData.prices[i].toString()} (1e18 USD)`);
  }
  ...
  for (let i = 0; i < metrics.volsBps.length; i++) {
    ...
    runtime.log(`    ${AssetNames[i]}: ${metrics.volsBps[i]} bps (${volPercent}%)`);
  }

  runtime.log("  Correlations (bps):");
  const pairNames = [
    "BTC-ETH", "BTC-LINK", "BTC-sDAI", "BTC-SHIB",
    "ETH-LINK", "ETH-sDAI", "ETH-SHIB",
    "LINK-sDAI", "LINK-SHIB",
    "sDAI-SHIB"
  ];
  for (let i = 0; i < metrics.corrsBps.length; i++) {
    const corrPercent = (metrics.corrsBps[i] / 100).toFixed(2);
    runtime.log(`    ${pairNames[i]}: ${metrics.corrsBps[i]} bps (${corrPercent})`);
  }
  ...
  if (writeResult.txStatus !== TxStatus.SUCCESS) {
    throw new Error(`Failed to write report: ${writeResult.errorMessage || writeResult.txStatus}`);
  }
