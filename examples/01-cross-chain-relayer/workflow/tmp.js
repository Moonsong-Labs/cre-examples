...
function unsafeIsSet(target, field) {
	...
	if (field.presence != IMPLICIT) {
		return target[name] !== undefined && Object.hasOwn(target, name);
	}
...
function unsafeIsSetExplicit(target, localName) {
	return Object.hasOwn(target, localName) && target[localName] !== undefined;
}
...
class ReflectMapImpl {
	...
	delete(key) {
		...
		const has = Object.hasOwn(this.obj, k);
		if (has) {
			delete this.obj[k];
		}
		return has;
	...
	has(key) {
		return Object.hasOwn(this.obj, mapKeyToLocal(key));
	}
	*keys() {
...
function anyFromJson(any, json, opts) {
	...
	if (typeName.startsWith("google.protobuf.") && Object.hasOwn(json, "value")) {
...
var EnclaveActionInputSchema = /* @__PURE__ */ messageDesc(
	file_capabilities_networking_confidentialhttp_v1alpha_client,
	4,
);
var HTTPEnclaveResponseDataSchema = /* @__PURE__ */ messageDesc(
	file_capabilities_networking_confidentialhttp_v1alpha_client,
	5,
);
...
class ClientCapability2 {
	...
	sendRequests(...args) {
		if (typeof args[1] === "function") {
			...
			return this.sendRequestsSugarHelper(
				runtime2,
				fn,
...
var ConfigSchema = /* @__PURE__ */ messageDesc(
	file_capabilities_networking_http_v1alpha_trigger,
	0,
);
var PayloadSchema = /* @__PURE__ */ messageDesc(
	file_capabilities_networking_http_v1alpha_trigger,
	1,
);
var KeyType;
...
var ConfigSchema2 = /* @__PURE__ */ messageDesc(
	file_capabilities_scheduler_cron_v1_trigger,
	0,
);
var PayloadSchema2 = /* @__PURE__ */ messageDesc(
	file_capabilities_scheduler_cron_v1_trigger,
	1,
);
...
function write(buffer, value, offset, isLE, mLen, nBytes) {
	...
	if (((value = Math.abs(value)), isNaN(value) || value === 1 / 0))
		...
		if (
			((e = Math.floor(Math.log(value) / Math.LN2)), value * (c = 2 ** -e) < 1)
		)
			e--, (c *= 2);
		if (e + eBias >= 1) value += rt / c;
		else value += rt * 2 ** (1 - eBias);
		if (value * c >= 2) e++, (c /= 2);
		if (e + eBias >= eMax) (m = 0), (e = eMax);
		else if (e + eBias >= 1) (m = (value * c - 1) * 2 ** mLen), (e = e + eBias);
		else (m = value * 2 ** (eBias - 1) * 2 ** mLen), (e = 0);
	}
	for (
		;
		mLen >= 8;
...
Buffer2.from = (value, encodingOrOffset, length) =>
...
Buffer2.alloc = (size, fill, encoding) => alloc(size, fill, encoding);
function allocUnsafe(size) {
	return assertSize(size), createBuffer(size < 0 ? 0 : checked(size) | 0);
}
Buffer2.allocUnsafe = (size) => allocUnsafe(size);
Buffer2.allocUnsafeSlow = (size) => allocUnsafe(size);
...
Buffer2.isBuffer = (b) =>
	b != null && b._isBuffer === true && b !== Buffer2.prototype;
Buffer2.compare = (a, b) => {
...
Buffer2.prototype.readIntLE = function (offset, byteLength2, noAssert) {
	...
	let val = this[offset],
		...
		i2 = 0;
	while (++i2 < byteLength2 && (mul *= 256)) val += this[offset + i2] * mul;
	if (((mul *= 128), val >= mul)) val -= 2 ** (8 * byteLength2);
	return val;
};
...
Buffer2.prototype.writeUint16LE = Buffer2.prototype.writeUInt16LE = function (
	...
	return (
		(this[offset] = value & 255), (this[offset + 1] = value >>> 8), offset + 2
	);
};
Buffer2.prototype.writeUint16BE = Buffer2.prototype.writeUInt16BE = function (
	...
	if (((value = +value), (offset = offset >>> 0), !noAssert))
		checkInt(this, value, offset, 2, 65535, 0);
	return (
		(this[offset] = value >>> 8), (this[offset + 1] = value & 255), offset + 2
	);
};
...
class NetworkLookup {
	...
	find(options) {
		...
		if (chainFamily && chainSelector !== undefined) {
			...
			let network248 = getBySelector(
				this.maps.testnetBySelectorByFamily[chainFamily],
			);
			if (!network248) {
				network248 = getBySelector(
